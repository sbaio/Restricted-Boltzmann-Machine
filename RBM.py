...
class RBM(object):
    ...
    def gradient_log_proba(self, vis_state, show=False, epoch=0, im_number=0):
        ...
        if show:
            show_images.showImage(show_images.vec_to_image(new_vis_state), epoch=epoch, im_number=im_number)
    ...
    def train(self, dateset, outputJson):
        ...
        images_to_show = [0, 72, 226, 2453, 2764, 2772, 31227, 44983]
        ...
        while current_epoch < self.max_epoch + 1 :
            ...
            for i in range(n_batch):
                ...
                for j, vis_state in enumerate(trainset[(i) * self.batchsize : min((i+1) * self.batchsize, len(trainset))]):
                    im_number = i + j*n_batch
                    vis_state = np.reshape(vis_state, (-1, 1))
                    log_proba_batch += self.log_proba(vis_state)
                    (dW, dhbias, dvbias) = self.gradient_log_proba(vis_state, show=((im_number in images_to_show)*(current_epoch%100==0)),
                                                                   epoch=current_epoch, im_number=im_number)
                    dW_batch += dW.T
                    dhbias_batch += dhbias
                    dvbias_batch += dvbias
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--learning_rate', dest='lrate', type=float, default=0.01, help='The step of gradient descent')
    ...
    parser.add_argument('--max_epoch', dest='mepoch', type=int, default=2000, help='Number of maximum epochs during the train')
